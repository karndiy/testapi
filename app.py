...
class TestPatientInfo(unittest.TestCase):
    def setUp(self):
        ...
        self.fake_data = {
            ...
            "gender": fake.random_element(elements=("ชาย", "หญิง", "อื่น ๆ")),
            ...
            "email": fake.email(),
            "note": fake.sentence(),
            "picture_base64": "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/wcAAwAB/0p8qOUAAAAASUVORK5CYII="
        }
    ...
    def generate_patient_data(self, cid=None,hn=None,vn=True):
        ...
        if hn == True: 
            _hn = str(fake.random_number(digits=4, fix_len=True))


        if vn == True: 
           _vn =  str(fake.random_number(digits=4, fix_len=True))
    ...
    def test_patient_data_structure(self):
        ...
        expected_keys = {
            ...
            "gender", "nationality", "address", "postcode", "phone",
            "email", "note", "picture_base64"
        }
...
def create_mock_patient_data():
    ...
    patients_data = {
        "1111111111111": {
            "message": "success",
            "data": test_instance.generate_patient_data("1111111111111",True,False)
        },
        "2222222222222": {
            "message": "success",
            "data": test_instance.generate_patient_data("2222222222222",False,True)
        },
        "3333333333335": {
            "message": "success",
            "data": test_instance.generate_patient_data("3333333333335",True,True)
        },
        "2222222222227": {
            "message": "success",
            "data": test_instance.generate_patient_data("2222222222227",True,True)
        },
        "3440300015187": {
            "message": "success",
            "data": test_instance.generate_patient_data("3440300015187",True,False)
        },
        "3102100818892": {
            "message": "success",
            "data": test_instance.generate_patient_data("3102100818892",False,True)
        }
    }

    return patients_data

patients_data = create_mock_patient_data()

# Define the fields for the patient information model
getPatientInfo_models = api.model('getPatientInfos', {
    'status': fields.Nested(api.model('Status', {
        'message': fields.String,
        'timestamp': fields.String
    })),
    'patient': fields.Nested(api.model('Patient', {
        'id': fields.String,
        ...
        'birth_details': fields.Nested(api.model('BirthDetails', {
            'dob': fields.String,
            'nationality': fields.String,
            'religion': fields.String
        })),
        'contact_info': fields.Nested(api.model('ContactInfo', {
            'email': fields.String,
            'mobile': fields.String,
            'telephone': fields.String,
            'address': fields.Nested(api.model('Address', {
                'address_no': fields.String,
        ...
        'medical_info': fields.Nested(api.model('MedicalInfo', {
            'hn': fields.String,
            'icd10': fields.String,
            'blood_type': fields.String,
            'medical_condition': fields.String,
            'allergy': fields.String,
            'underlying_disease': fields.String,
            'medication_detail': fields.String,
            'risk_factor': fields.String,
            'treatment_department': fields.String,
    ...
    'pictures': fields.Nested(api.model('Pictures', {
        'profile_picture_url': fields.String,
        'picture_ids': fields.List(fields.String)
    })),
    'other_details': fields.Nested(api.model('OtherDetails', {
        'company': fields.String,
        'branch': fields.String,
        'department': fields.String,
        'note': fields.String,
        'trust_the_nation': fields.String
    }))
})
...
getPatientInfo_model = api.model('getPatientInfo', {
    ...
    'nationality': fields.String(description="สัญชาติของผู้ป่วย"),
    'address': fields.String(description="หมายเลขที่อยู่ เช่น บ้านเลขที่"),
    'postcode': fields.String(description="รหัสไปรษณีย์ที่อยู่"),
    'phone': fields.String(description="หมายเลขโทรศัพท์มือถือของผู้ป่วย"),
    'email': fields.String(description="อีเมลของผู้ป่วย"),    
    'note': fields.String(description="หมายเหตุเพิ่มเติมเกี่ยวกับผู้ป่วย"),
    'picture_base64': fields.String(description="รูปภาพของผู้ป่วยในรูปแบบ base64"),
})


# Define PatientInfo_data as a response model
PatientInfo_data = api.model('PatientInfoData', {
    'message': fields.String(description='Status message for the response'),
...
vital_sign_data = api.model('VitalSignData', {
    ...
    "statuscode":fields.String(description="Status code", example= StatusCode.CREATED),
    "data": fields.Nested(api.model('VitalSignDetails', {
        ...
        "bp": fields.String(description="Blood Pressure", example="120/80"),
...
send_vital_sign = api.model('SendVitalSign', {
    ...
    'bp': fields.String(description='Blood Pressure'),
...
@api.route('/gateway/sendvitalsign')
class SendVitalSignResource(Resource):
    ...
    def post(self):
        ...
        print(input_data)
        if not input_data:
            return {"error": "not found JSON data"}, 400
...
@api.route('/gateway/sendtransaction')
...
@api.route('/gateway/patient')
class GetPatientInfoResource(Resource):
    
    @api.doc(description="Retrieve patient information based on CID")
    @api.param('cid', 'The CID of the patient to retrieve information for', required=False)
    @api.param('hn', 'The HN of the patient to retrieve information for', required=False)
    ...
    def get(self):
        ...
        if not str(cid) or not str(hn):
            return {"message": "not found cid or hn patient","data":""}, 400

        # Check if the CID exists in the mock data
        #patient_info = patients_data.get(cid)

        if cid:
            patient_info = patients_data.get(str(cid) )
        elif hn:
            patient_info = patients_data.get(str(hn))

        if not patient_info:
            return {"message": "not found patient","data":""}, 404
